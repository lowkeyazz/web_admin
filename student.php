...
<html>
  ...
  <body>
    <header class="mainstu">
      ...
      <div class="D2">
        <img class="I1" src="image/icontil.png" />
	...
	<?php
		...
		if(isset($_POST["sign_in"]))
		{
			...
			$select_user="SELECT * FROM etudiant WHERE username='$username' AND password='$password'";
			...
			if($check_user==1)
			{
				...
				$get_user="select * from etudiant where username='$user'";
				$run_user=mysqli_query($con, $get_user);
				$row=mysqli_fetch_array($run_user);
				$nom=$row['Nom'];
				echo"<script>window.open('etudiant_interface.php?username=$nom','_self') </script>";
